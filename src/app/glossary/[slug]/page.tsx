...
export async function generateStaticParams() {
    return getAllEntries("en").map((e) => ({ slug: e.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const entry = getEntryBySlug("en", params.slug);
    if (!entry) return { title: "Not Found" };
    return {
        title: `${entry.term} — Imoifa Glossary`,
        description: entry.definition,
    };
}

export default function GlossarySlugPage({ params }: Props) {
    const entry = getEntryBySlug("en", params.slug);
    if (!entry) notFound();

    const yoEntry = getEntryBySlug("yo", params.slug);
    const relatedEntries = getRelatedEntries("en", entry.related ?? []);
    ...
    return (
        <div className="max-w-4xl mx-auto px-6 py-12">
            ...
            <nav className="mb-10 text-xs font-mono text-parchment-muted flex items-center gap-2 flex-wrap">
                ...
                <Link href="/glossary" className="hover:text-parchment transition-colors">Glossary</Link>
                <span className="text-gold/30">/</span>
            ...
            {relatedEntries.length > 0 && (
                <section className="mb-10">
                    <p className="section-label mb-4">Related Terms</p>
            ...
            {yoEntry && (
                <div className="mt-12 p-5 border border-gold/20 bg-gradient-to-r from-gold/5 to-transparent rounded-sm flex items-start justify-between gap-4 flex-wrap">
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-parchment font-serif text-lg mb-0.5">{yoEntry.term}</p>
                        <p className="text-parchment-dim text-sm line-clamp-2">{yoEntry.definition}</p>
                    </div>
                    <Link href={`/yo/glossary/${entry.slug}`} className="btn-ghost shrink-0 self-center">
                        Ẹ ka ní Yorùbá →
                    </Link>
                </div>
            )}
            ...
            <div className="mt-12 pt-8 border-t border-gold/10">
                <Link href="/glossary" className="text-parchment-muted text-sm font-mono hover:text-parchment transition-colors">
                    ← Back to Glossary
                </Link>
            </div>
        </div>
